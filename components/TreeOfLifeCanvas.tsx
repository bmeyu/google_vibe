...
const LOCK_DISTANCE_THRESHOLD = 0.55;
const UNLOCK_DISTANCE_THRESHOLD = 0.25;
const LOCK_TIME_REQUIRED = 30;
const STRING_SMOOTHING = 0.2;
const STRING_GAP = 25;
const VIBRATION_DECAY = 0.85;
const MAX_SWIRLS = 16;
const NOTE_SYMBOLS = ['♪', '♫', '♬', '✦', '★', '✶'];
...
export const TreeOfLifeCanvas: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const initMaterial = (texture: THREE.Texture) => {
        ...
        // Calculate plane size to maintain image aspect ratio
    ...
    const createFallbackTexture = (): THREE.Texture => {
        ...
        if (ctx) {
            // Golden/amber gradient for Tree of Life
    ...
    const handleSuccess = (texture: THREE.Texture) => {
        console.log('[TreeOfLife] Texture loaded successfully:', texture.image?.width, 'x', texture.image?.height);
        if (textureLoadHandled) return;
        textureLoadHandled = true;
        initMaterial(texture);
    };
    const handleError = (err?: any) => {
        console.error('[TreeOfLife] Texture load error:', err);
        if (textureLoadHandled) return;
        textureLoadHandled = true;
        initMaterial(createFallbackTexture());
    };
    try {
        console.log('[TreeOfLife] Loading texture from:', CURRENT_THEME.backgroundUrl);
        loader.load(CURRENT_THEME.backgroundUrl, handleSuccess, undefined, handleError);
    } catch (e) {
        console.error('[TreeOfLife] Exception during load:', e);
        handleError();
    }
    const timeoutId = setTimeout(() => {
        if (!textureLoadHandled) {
            console.warn('[TreeOfLife] Texture load timeout, using fallback');
            handleError();
        }
    }, 10000);
    const handleResize = () => {
    ...
    return () => {
      ...
      renderer.dispose();
      // Reset refs on cleanup to handle React StrictMode double-mount
  ...
  const drawCosmicString = (ctx: CanvasRenderingContext2D, p1: Point, p2: Point, color: string, vibrationAmp: number) => {
    ...
    ctx.lineWidth = 3 + (vibrationAmp * 0.2);
    ctx.shadowBlur = 10 + vibrationAmp;
                            ...
                            currentStringLines.forEach((line) => {
                               if (segmentsIntersect(line.p1, line.p2, prevOverlay, pOverlay)) {
                                   if (now - lastPluckTimeRef.current > PLUCK_COOLDOWN) {
                                      ...
                                      const vibIndex = line.index === 1 ? 0 : line.index === 0 ? 1 : 2;
                                      stringVibrationRef.current[vibIndex] = 20;
    ...
    <div className="relative w-full h-full bg-black overflow-hidden font-sans select-none">
      ...
      <div className="absolute bottom-10 left-0 right-0 pointer-events-none z-20 flex justify-center px-4">
         <div className="bg-gradient-to-t from-black/80 to-black/40 backdrop-blur-md border-t border-amber-500/20 px-10 py-5 rounded-2xl text-center transform transition-all duration-500 hover:bg-black/90 shadow-2xl max-w-lg">
            <h3 className="text-amber-100/90 font-serif text-xl tracking-wide mb-2 italic">The Golden Strings</h3>
            <p className="text-white/70 text-sm font-light leading-relaxed">
              ...
              {isStringsActive && (
                  <>
                    <span className="block mb-1">Pluck strings to play music and awaken the spirals.</span>
